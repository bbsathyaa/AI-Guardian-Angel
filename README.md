# AI-Guardian-Angel — SOS AI Emergency Agent


This repo contains a Cloud Run-deployable FastAPI backend that accepts emergency triggers (text + location), classifies intent & severity with Vertex AI, looks up nearby services with Google Maps, notifies emergency contacts (Twilio/FCM hooks), and logs incidents to Firestore.


See `app/` for the FastAPI app, `infra/` for Terraform and gcloud commands, `scripts/` for deploy scripts, and `.github/` for CI/CD.
